    ...
    for (int i = 0; i < token_per_reduce; i++) {
      ...
      for (int j = 0; j < ele_per_thread; j++) {
        ...
        if (mask_future && ele_per_thread * threadIdx.x + j > token_id + i) {
          temp_val = REDUCE_FLOAT_INF_NEG; //masking set to infinity so that softmax will assign zero
        } else {
        ...
        val[i][j] = temp_val;
        l_max[i] = fmaxf(l_max[i], temp_val); //max is needed to stabilize the calculation later
      }
    }
    // END ASSIGN3_1
    // warp reduce max
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  // BEGIN ASSIGN3_1
  int out_grad_size = rows * softmax_len * sizeof(float);
  int soft_inp_size = rows * softmax_len * sizeof(float);

  float *d_out_grad;
  const float *d_soft_inp;
  ...
  cudaMemcpy((void *)d_soft_inp, soft_inp, soft_inp_size, cudaMemcpyHostToDevice);
  ...
  cudaMemcpy((void *)soft_inp, d_soft_inp, soft_inp_size, cudaMemcpyDeviceToHost);
