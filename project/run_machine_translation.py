...
def get_dataset(dataset_name, model_max_length):
    ...
    """
    dataset = {
        split: datasets.load_dataset("bbaaaa/iwslt14-de-en-preprocess", split=split)[
            "translation"
        ]
...
def generate(
    ...
    for example in tqdm.tqdm(examples, desc=f"Generating {desc}"):
        ...
        while len(token_ids) <= model_max_length:
            ...
            # print(len(token_ids))
            token_ids_tensor = minitorch.tensor(token_ids, backend=backend).view(
                *(1, len(token_ids))
            )
            ...
            # print(logits.shape)
            # print(len_src)
            next_token_logits = logits.to_numpy()[0, len(token_ids) - 1, :]
            gen_id = np.argmax(next_token_logits, axis=-1)
            # print(gen_id)
