...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    max_vals = max(input, dim = dim)
    diff = input - max_vals

    return max_vals + diff.exp().sum(dim=dim).log()
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    # print(logits[torch.arange(logits.shape[0]), indices])
    logits
    result = logsumexp(logits, dim = 1) - (logits * one_hot(target, logits.shape[1])).sum(dim=1)
