...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
        ...
        rows = out_grad.shape[0] * out_grad.shape[1] * out_grad.shape[2]
        softmax_len = soft_inp.shape[3]
    ...
    def layernorm_bw(
        ...
        gamma_grad = gamma.zeros((1, gamma.shape[0]))
        beta_grad = beta.zeros((1, beta.shape[0]))
    ...
    def flash_attention_causal_bw(
        ...
        #     requires_grad=True,
        # )
        lib_flashattention.launch_flashattention_backward_causal.argtypes = [
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags="C_CONTIGUOUS"),
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags="C_CONTIGUOUS"),
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags="C_CONTIGUOUS"),
            np.ctypeslib.ndpointer(dtype=datatype, ndim=1, flags="C_CONTIGUOUS"),
        ...
        lib_flashattention.launch_flashattention_backward_causal.restype = None
        ...
        lib_flashattention.launch_flashattention_backward_causal(
            Q._tensor._storage,
            K._tensor._storage,
            V._tensor._storage,
            O._tensor._storage,
